...
sub _onPlayerCommand {
    ...
    if ($cmd eq 'play') {
        ...
        # Detect natural end using our stored last known track position
        # (controller elapsed resets to 0 when track ends so can't be used directly)
        my $naturalEnd = 0;
        eval {
            my $song = $client->controller()->playingSong();
            if ($song) {
                my $duration     = $song->duration() // 0;
                my $elapsed      = $playerState{$id}{lastTrackTime} // 0;
                ...
                if ($duration > 0 && ($duration - $elapsed) <= 10 && $lastTrack) {
                    $naturalEnd = 1;
