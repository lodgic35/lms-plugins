...
sub _onPlayerCommand {
    ...
    if ($cmd eq 'play') {
        ...
        # Distinguish natural playlist end from user deliberately stopping
        # When playlist ends naturally LMS clears it so count = 0
        my $playlistCount = Slim::Player::Playlist::count($client) // 0;
        if ($playlistCount == 0) {
            # Natural end - suppress recovery but allow silence timer
            $playerState{$id}{userStopped}            = 0;
            $playerState{$id}{retries}                = 0;
            $playerState{$id}{hangDetected}           = 0;
            $playerState{$id}{byteStall}              = 0;
            ...
            # User deliberately stopped - suppress everything
            $playerState{$id}{userStopped}            = 1;
            $playerState{$id}{retries}                = 0;
            $playerState{$id}{hangDetected}           = 0;
            $playerState{$id}{byteStall}              = 0;
...
sub _checkPlayer {
    ...
    # Skip recovery if playlist is empty - natural playlist end
    # Silence timer handles this case instead
    my $playlistCount = Slim::Player::Playlist::count($client) // 0;
    if ($playlistCount == 0) {
        $log->debug("[$id] Playlist empty - skipping recovery, silence timer will handle");
        return;
    }
