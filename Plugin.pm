...
sub _onPlayerCommand {
    ...
    if ($cmd eq 'play') {
        ...
        # Detect natural end by comparing track position to track duration
        # If track position is within 5 seconds of the end it played to completion
        my $naturalEnd = 0;
        eval {
            my $song     = $client->controller()->playingSong();
            if ($song) {
                my $duration = $song->duration() // 0;
                my $elapsed  = $client->controller()->playingSongElapsed() // 0;
                # Also check if we're at the last track in the playlist
                my $trackIndex   = Slim::Player::Source::streamingSongIndex($client) // 0;
                my $playlistSize = Slim::Player::Playlist::count($client) // 0;
                my $lastTrack    = ($trackIndex >= $playlistSize - 1);
                $log->debug("[$id] Stop check: elapsed=${elapsed}s duration=${duration}s trackIndex=$trackIndex playlistSize=$playlistSize lastTrack=$lastTrack");
                if ($duration > 0 && ($duration - $elapsed) <= 5 && $lastTrack) {
                    $naturalEnd = 1;
                    $log->debug("[$id] Natural end detected: elapsed=${elapsed}s duration=${duration}s");
                }
            } else {
                $log->debug("[$id] Stop check: no playing song object available");
            }
        };
        $log->debug("[$id] Natural end eval error: $@") if $@;

        if ($naturalEnd) {
            # Playlist played to end - allow silence timer
            $playerState{$id}{userStopped} = 0;
            $log->debug("[$id] Playlist ended naturally - silence timer allowed");
        } else {
