...
sub _onPlayerCommand {
    ...
    if ($cmd eq 'play') {
        ...
        $playerState{$id}{stopPlaylistIndex}      = undef;
        $playerState{$id}{stopPlaylistSize}       = undef;
        ...
        # Stop/clear is issued by LMS internally (playlist end, Spotify failure)
        # NOT by the user (user only has pause in the UI)
        # So we suppress recovery but allow silence timer
        $playerState{$id}{userStopped}            = 0;
        $playerState{$id}{retries}                = 0;
        $playerState{$id}{hangDetected}           = 0;
        $playerState{$id}{byteStall}              = 0;
        ...
        $playerState{$id}{usingFallback}          = 0;
        $playerState{$id}{silenceHandoverPending} = 0;
        # Store playlist position at time of stop for natural end detection
        $playerState{$id}{stopPlaylistIndex} = Slim::Player::Source::streamingSongIndex($client) // 0;
        $playerState{$id}{stopPlaylistSize}  = Slim::Player::Playlist::count($client) // 0;
        $log->debug("[$id] Stop/clear - silence timer allowed");
        ...
        $playerState{$id}{silenceStart}          = undef;
        _recordTrackInfo($client);
        $log->debug("[$id] Track index change - watchdog reset");

        # Check after a short delay if playback actually started
        # If not, playlist was exhausted - start silence timer
        Slim::Utils::Timers::setTimer(
            $client,
            time() + 3,
            sub {
                ...
                if ($mode ne 'play') {
                    $playerState{$cid}{stopPlaylistIndex} = Slim::Player::Source::streamingSongIndex($c) // 0;
                    $playerState{$cid}{stopPlaylistSize}  = Slim::Player::Playlist::count($c) // 0;
                    $log->debug("[$cid] Index did not start playback - playlist exhausted, silence timer will run");
                }
            }
        );
    }
...
sub _checkPlayer {
    ...
    # Also treat debounce phase (stopTime set) as recovery in progress
    # UNLESS playlist is empty/exhausted - then silence timer should handle it
    my $retries       = $playerState{$id}{retries} // 0;
    my $playlistCount = Slim::Player::Playlist::count($client) // 0;
    my $inRecovery    = $retries > 0 || (defined $playerState{$id}{stopTime} && $playlistCount > 0);

    # --- Silence timeout ---
    # Runs after stop/clear (playlist end or Spotify failure)
    # Does NOT run when user has paused (userStopped = 1)
    # Pauses during active recovery or cooldown
    if ($prefs->get('silence_enabled') && $prefs->get('silence_fallback_url')
        && $playerState{$id}{lastPlayTime}
        && !$playerState{$id}{userStopped}) {
        if (!$inRecovery && !$inCooldown) {
            ...
            $playerState{$id}{silenceStart} = undef;
            $log->debug("[$id] Silence timer paused (recovery in progress)") if $inRecovery;
        }
    }

    # Skip recovery if user deliberately stopped or never played
    return if $playerState{$id}{userStopped};
    return unless $playerState{$id}{lastPlayTime};

    # Skip recovery if the stopped track was the last in the playlist
    # (cur_index >= tracks - 1 means it was the final track)
    my $stopIndex = $playerState{$id}{stopPlaylistIndex} // -1;
    my $stopSize  = $playerState{$id}{stopPlaylistSize}  // 0;
    if ($stopSize > 0 && $stopIndex >= $stopSize - 1) {
        $log->debug("[$id] Last track ended (index=$stopIndex size=$stopSize) - silence timer handles this");
        return;
    }
...
sub _switchToFallback {
    ...
    my $title  = $prefs->get('fallback_title') || 'Fallback Preset';
