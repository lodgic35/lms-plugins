        ...
        Slim::Utils::Timers::setTimer(
            ...
            sub {
                ...
                if ($mode ne 'play') {
                    ...
                    # userStopped already 0, silence timer will start on next poll
                }
            }
        );
    }
...
sub _checkPlayer {
    ...
    # Skip recovery if playlist is empty - it ended naturally
    # Silence timer handles this case
    if ($playlistCount == 0) {
        $log->debug("[$id] Playlist empty - silence timer will handle");
        return;
    }
