...
my %playerState;
my %watchdogAction;  # track when watchdog itself issues commands
...
sub _onPlayerCommand {
    ...
    if ($cmd eq 'play') {
        # Skip if this play was issued by the watchdog itself
        if ($watchdogAction{$id}{play}) {
            $watchdogAction{$id}{play} = 0;
            $log->debug("[$id] Watchdog-issued play - not resetting state");
            return;
        }
        ...
        return if $watchdogAction{$id}{stop};  # ignore if watchdog issued it
...
sub _checkForHang {
    ...
    # If already waiting for internet, handle here instead of re-triggering hang
    if ($playerState{$id}{waitingForInternet}) {
        if (_internetUp()) {
            ...
            $log->info("[$id] Still waiting for internet...");
        }
        return;
    }
...
sub _switchToFallback {
    ...
    # favid is now the direct stream URL from the OPML file
    $client->execute(['playlist', 'clear']);
    $client->execute(['playlist', 'play', $favid, $title]);
...
sub getFavourites {
    ...
    # Read directly from the favourites OPML file - more reliable than the API
    my $opmlFile = Slim::Utils::Prefs::dir() . "/favorites.opml";

    unless (-f $opmlFile) {
        $log->warn("Favourites OPML file not found: $opmlFile");
        return $favs;
    }

    open(my $fh, '<', $opmlFile) or do {
        $log->warn("Cannot open favourites file: $!");
        return $favs;
    };
    ...
    while (my $line = <$fh>) {
        if ($line =~ /URL="([^"]+)"[^>]*text="([^"]+)"/) {
            ...
            $log->info("Found favourite: $title");
        }
    }
    close($fh);

    $log->info("Favourites: found " . scalar(@{$favs}) . " items from OPML");
