...
# StreamWatchdog - LMS Plugin v1.4
...
sub initPlugin {
    ...
    $log->info("Stream Watchdog v1.6 initialised. Poll: "
...
# ---------------------------------------------------------------------------
# Internet check - ping via system call
# Returns 1 if internet is up, 0 if down
# ---------------------------------------------------------------------------
sub _internetUp {
...
sub _checkPlayer {
    ...
    # If we were waiting for internet and now it's back up, try to resume
    if ($playerState{$id}{waitingForInternet} && $mode ne 'play') {
        if (_internetUp()) {
            $log->info("[$id] Internet restored - attempting Spotify resume");
            $playerState{$id}{waitingForInternet} = 0;
            $playerState{$id}{retries}            = 0;
            $playerState{$id}{stopTime}           = undef;
            $client->execute(['play']);
...
# ---------------------------------------------------------------------------
# Recovery dispatcher - checks internet first
# ---------------------------------------------------------------------------
sub _attemptRecovery {
    ...
    if (!$online) {
        # Internet is down - wait patiently
        $playerState{$id}{waitingForInternet} = 1;
        $log->warn("[$id] Internet is DOWN - waiting for connection to restore (reason: $reason)");
        return;
    }

    # Internet is up - proceed with normal recovery
    $log->info("[$id] Internet is UP - proceeding with recovery (reason: $reason, source: $source)");

    if ($retries >= $maxRetries) {
        # Max retries exhausted with internet up - switch to fallback radio
        if ($prefs->get('fallback_enabled') && $prefs->get('fallback_url')) {
            _switchToFallback($client);
        } else {
            # No fallback configured - just cooldown
...
# ---------------------------------------------------------------------------
# Get favourites list for settings page dropdown
# ---------------------------------------------------------------------------
sub getFavourites {
    my $favs = [];

    # Use synchronous request to fetch favourites
    ...
    for my $item (@{$items}) {
        # Favourites use id field - include all items that have an id
        next unless $item->{id};
        # Skip folders/containers (hasitems means it contains sub-items)
    ...
    $log->debug("Favourites: found $count items, " . scalar(@{$favs}) . " audio items");
