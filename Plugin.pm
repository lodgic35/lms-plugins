...
sub _recordTrackInfo {
    ...
    $playerState{$id}{lastPlaylistSize}  = Slim::Player::Playlist::count($client) // 0;
...
sub _checkPlayer {
    ...
    # Use values recorded DURING playback - LMS resets index to 0 when stop fires
    my $stopIndex = $playerState{$id}{lastPlaylistIndex} // -1;
    my $stopSize  = $playerState{$id}{lastPlaylistSize}  // 0;
